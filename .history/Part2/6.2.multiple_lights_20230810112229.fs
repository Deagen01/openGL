...
void main()
{    
    material.diffuse = vec3(mix(texture(diffuseMap, TexCoords),texture(dacoMap, TexCoords),0.5));
    // properties
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient = light.ambient * vec3(texture(, TexCoords));

...
void main()
{
    ...
    /*Ambient */
    vec3 ambient = light.ambient * vec3(texture(specularMap, TexCoords));   /*use specular texture */
    
    /*Diffuse */
    vec3 norm = normalize(Normal);
    vec3 lightDir = normalize(light.position - FragPos);
    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = light.diffuse * diff * vec3(texture(specularMap, TexCoords));   /*use specular texture */

...
void main()
{
    ...
    vec3 emission_map_value = vec3(texture(emissionMap, TexCoords).rgb);
    ...
    // vec3 specular = light.specular * spec * (vec3(1.0)-specular_map_value);  
    // emission
    vec3 emission = emission_map_value;
    vec3 result = ambient + diffuse + specular +emiission;
